...
class EvdevBackend(InputBackend):
    ...
    def _handle_device_error(self, device, error):
        ...
        if isinstance(error, OSError) and error.errno in (errno.EBADF, errno.ENODEV):
    ...
    def on_input_event(self, event: tuple[KeyCode, InputEvent]):
        """Callback method to be overridden by the KeyListener. This method is called for each processed input event."""
        pass
